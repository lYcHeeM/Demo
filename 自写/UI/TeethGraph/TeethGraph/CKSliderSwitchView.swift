...
class CKSlideSwitchView: UIView {
    ...
    let  rootScrollView          : UIScrollView = {
        ...
        rootScrollView.backgroundColor                = UIColor(white: 0, alpha: 0.05)
...
extension CKSlideSwitchView {
    ...
    fileprivate func createTabItemButtons() {
        ...
        for index in 0..<tabItemCount {
            let tabItem: UIButton = UIButton(type: .system)
    ...
    fileprivate func configRootScrollView(_ runType: CKSlideSwitchViewRunType) {
        ...
        if tabItemCount == 3 || tabItemCount == 4 {
            rootScrollView.contentSize = CGSize(width: CGFloat(tabItemCount) * rootScrollView.frame.width, height: rootScrollView.frame.height)
            ...
            if rootScrollViewCurrentPage != 0 && rootScrollViewCurrentPage != tabItemCount - 1 {
                rootScrollView.contentSize = CGSize(width: 3 * rootScrollView.frame.width, height: rootScrollView.frame.height)
            } else {
                rootScrollView.contentSize = CGSize(width: 2 * rootScrollView.frame.width, height: rootScrollView.frame.height)
            }
            ...
            if rootScrollViewCurrentPage != 0 {
                ...
                if tabItemCount <= 1 {
                    rootScrollView.contentSize = CGSize(width: rootScrollView.frame.width, height: rootScrollView.frame.height)
                }
            }
        }
    }
